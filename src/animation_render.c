...
static void pre_loop(t_game *container) {
    ...
    container->windowRect.x = 886;
	container->windowRect.y = 95;
	container->windowRect.w = 650;
	container->windowRect.h = 865;
    ...
    container->textureRect.w /= 22;
}

void animation_render(t_game *container) {
    if (container->current_scene_1 > 6) {
        pre_loop(container);
            int totalFrames3 = 22;
            ...
            SDL_RenderClear(container->renderer);
            //SDL_UpdateWindowSurface(container->window);
    }
